"""Closed, app-owned internal seam for optional capture connectors.

This module is intentionally an application extension contract, not a public
connector SDK. Discovery is metadata-only until an explicitly allow-listed,
egress-enabled connector is requested. Allow-listed implementations are trusted
repository code: hostile Python reflection requires the separately deferred
isolated-worker runtime and is not represented as an in-process security boundary.
"""
...
class ConnectorRegistry:
    ...
    def _validated_entries(
        ...
        unlisted = set(names) - set(profile.allow_list)
        if unlisted:
            raise ConnectorDiscoveryError("connector entry point is not allow-listed")
