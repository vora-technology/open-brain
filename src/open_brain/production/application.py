"""Concrete local application composition for public command and service surfaces."""
...
from open_brain.cli.migrate import plan_migration
...
from open_brain.operations.runlog import RunMetadata, RunOutcome
from open_brain.operations.runlog_store import FilesystemRunLogStore
...
@dataclass(frozen=True, slots=True)
class ConfiguredMigrationFamily:
    work_root: Path
    state_root: Path

    def dispatch(self, argv: tuple[str, ...]) -> CommandDispatchResult:
        if len(argv) != 1:
            return _family_failure("migrate", "invalid_migration_request", usage=True)
        try:
            kind = MigrationKind(argv[0].replace("-", "_"))
        except ValueError:
            return _family_failure("migrate", "invalid_migration_request", usage=True)
        if kind is MigrationKind.CONTENT_LAYOUT:
            return _family_failure("migrate", "invalid_migration_request", usage=True)
        result = plan_migration(
            kind=kind,
            vault_root=self.work_root,
            state_root=self.state_root,
        )
        if result.plan is None:
            return result
        plan = result.plan.to_redacted_dict()
        plan.pop("ready", None)
        return _FamilyResult(
            result.exit_code,
            {
                "command": "migration",
                "plan": plan,
                "status": "planned" if result.plan.ready else "blocked",
            },
        )
...
def compose_production_application(
    *,
    ...
    dependencies = ProductionCommandDependencies(
        ...
        migrate=ConfiguredMigrationFamily(config.work_root, config.state_root),
...
def _family_failure(
    ...
    *,
    ...
    command = "migration" if family == "migrate" else family
    return _FamilyResult(
