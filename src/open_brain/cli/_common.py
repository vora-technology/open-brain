...
def _validate_public_output(
    ...
    *,
    ...
    if isinstance(value, str):
        ...
        if is_key and any(
            variant.casefold() in _RESERVED_READINESS_KEYS for variant in variants
        ):
            raise ValueError("reserved readiness key in command adapter output")
        if any(
            pattern.search(variant)
            for pattern in _PUBLIC_OUTPUT_RESIDUALS
            for variant in variants
        ):
            raise ValueError("unsafe command adapter output")
        if any(ord(character) < 32 and character not in {"\n", "\t"} for character in value):
            raise ValueError("unsafe command adapter output")
        check_argv = check_arguments and not (
            is_key and value in _PUBLIC_OUTPUT_SCHEMA_KEYS
        )
        if check_argv and any(
            ...
            and (
                variant == argument
                or (len(argument) >= 8 and argument in variant)
            )
