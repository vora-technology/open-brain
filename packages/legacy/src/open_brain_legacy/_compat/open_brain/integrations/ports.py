...
_RUNTIME_TRAVERSAL_PATTERN = re.compile(r"(?:^|[^A-Za-z0-9_.-])\.\.(?:$|[\\/])")
...
class RetrievalFeedbackReceipt:
    ...
    def __post_init__(self) -> None:
        if (
            not _is_opaque_id(self.retrieval_id)
            or not isinstance(self.outcome, FeedbackOutcome)
            or not _is_int_in_range(self.result_count, minimum=0, maximum=_MAX_RETRIEVAL_RESULTS)
...
class VaultWriteResult:
    ...
    def __post_init__(self) -> None:
        if (
            ...
            or not _is_int_in_range(self.bytes_written, minimum=0, maximum=_MAX_MARKDOWN_LENGTH * 4)
        ):
...
class ProviderSyncResult:
    ...
    def __post_init__(self) -> None:
        if (
            ...
            or (self.next_cursor_ref is not None and not _is_opaque_id(self.next_cursor_ref))
        ):
...
class PostCommitSignal:
    ...
    def __post_init__(self) -> None:
        if not all(
            _is_opaque_id(value) for value in (self.signal_id, self.repository_id, self.revision_id)
        ):
...
class HookEmitResult:
    ...
    def __post_init__(self) -> None:
        if not _is_opaque_id(self.signal_id) or not isinstance(self.status, HookSignalStatus):
...
class AuditFinding:
    ...
    def __post_init__(self) -> None:
        if not isinstance(self.field, RuntimeField) or not isinstance(self.code, AuditFindingCode):
...
def _is_input_text(value: object, *, maximum: int) -> bool:
    return isinstance(value, str) and 0 < len(value) <= maximum and not _has_unsafe_control(value)
