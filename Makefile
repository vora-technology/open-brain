...
dev:
	PYTHONPATH=packages/app/src:packages/engine/src:src uv run python -m open_brain --version
...
audit:
	...
	PYTHONPATH=packages/app/src:packages/engine/src:src uv run python -m open_brain.dev.release_audit --root . --private-denylist "$(PRIVATE_DENYLIST)"

audit-history:
	@test -n "$(PRIVATE_DENYLIST)" || (echo "PRIVATE_DENYLIST is required" >&2; exit 2)
	PYTHONPATH=packages/app/src:packages/engine/src:src uv run python -m open_brain.dev.public_history_audit --repository . --private-denylist "$(PRIVATE_DENYLIST)"
...
verify-artifacts: build
	PYTHONPATH=packages/app/src:packages/engine/src:src uv run python -m open_brain.dev.artifact_policy --policy release/v0-artifact-policy.json --artifacts dist/*
