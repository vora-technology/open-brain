...
The parser exposes 29 top-level command families:

`backup`, `capture`, `close-day`, `config`, `cron`, `curation`, `dev`, `digest`, `doctor`, `explain`, `hooks`, `index`, `ledger`, `lifeos`, `lint`, `messages`, `migrate`, `now`, `okf`, `ops`, `proposals`, `query`, `registry`, `retention`, `review`, `share`, `social`, `status`, and `ui`.
...
- `open-brain` → `open_brain.cli.composition:run`
