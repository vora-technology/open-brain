...
- P3-W6 reviewed candidates: `9854f77` passed the first review before Linux exposed five macOS-only roots; `2ce8101` passed full repaired gates and `P3-W6-REREVIEW-02` at `READY`, P0/P1/P2 `0/0/0`, 12/12 complete before the next Linux run exposed a stalled-client backlog race.
- Current subphase: add an explicit bounded Unix-socket listen backlog, retain the accepted-client timeout, record the Linux `EAGAIN` evidence, and use one D-020 mandatory read-only rereview after full gates pass.
- Next action: verify and commit the backlog repair, rerun every exact-candidate gate, dispatch `P3-W6-REREVIEW-03`, then update PR `#4` and require every check to pass.
